...
describe("Treasury Token Migration", async () => {
    before(async () => {
        ...
        await newTreasury.connect(deployer).enable(0, olympusTokenMigrator.address, olympusTokenMigrator.address);
    });
    ...
    it("Should fail if sender is not DAO", async () => {
        ...
        await expect(
            olympusTokenMigrator.conntect(user1).migrateContracts("_newTreasury", "_newStaking", "_newOHM", "_newsOHM")
