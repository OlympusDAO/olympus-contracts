...
describe.only("Bond Depository", async () => {
    ...
    // Initial mint for Frax, FLOOR and DAI (10,000,000)
    const initialMint = "10000000000000000000000000";
    const initialFLOOR = "10000000000000000";
    ...
    beforeEach(async () => {
        ...
        gFLOOR = await gFloorFactory.deploy("50000000000"); // Set index as 50
        staking = await smock.fake("IStaking");
        depository = await depositoryFactory.deploy(auth.address, floor.address, gFLOOR.address, staking.address, treasury.address);
        ...
        await floor.mint(deployer.address, "10000000000000")
        await treasury.baseSupply.returns(await floor.totalSupply());

        // Mint enough gFLOOR to payout rewards
        await gFLOOR.mint(depository.address, "1000000000000000000000")

        await floor.connect(alice).approve(depository.address, LARGE_APPROVAL);
    ...
    it("should not redeem before vested", async () => {
        let balance = await floor.balanceOf(bob.address);
        let amount = "10000000000000000000000"; // 10,000
        await depository.connect(bob).deposit(
            bid,
            amount,
        ...
        await expect(depository.redeem(bob.address, [0], true)).to.be.revertedWith("Depository: note not matured");
        expect(await floor.balanceOf(bob.address)).to.equal(balance);
    });
    ...
    it("should redeem after vested", async () => {
        ...
        await depository.redeem(bob.address, [0], true);
        
        const bobBalance = Number(await gFLOOR.balanceOf(bob.address));
        expect(bobBalance).to.greaterThanOrEqual(Number(await gFLOOR.balanceTo(expectedPayout)));
        expect(bobBalance).to.lessThan(Number(await gFLOOR.balanceTo(expectedPayout * 1.0001)));
    });

    it("should give correct rewards to referrer and dao", async () => {
        let daoBalance = await floor.balanceOf(deployer.address);
        let refBalance = await floor.balanceOf(carol.address);
        let amount = "10000000000000000000000"; // 10,000
        ...
        // Mint floor for depository to payout reward
        await floor.mint(depository.address, "1000000000000000000000");

        let daoExpected = Number(daoBalance) + Number(Number(payout) * daoReward / 1e4);
        await depository.getReward();

        const frontendReward = Number(await floor.balanceOf(deployer.address));
        ...
        const carolReward = Number(await floor.balanceOf(carol.address));
