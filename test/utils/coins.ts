// Ok, so here we're gonna categorize stuff according to type rather than protocol.
// Protocol categorization is in `protocols.ts`, you can add the gov tokens to the protocol,
// but this file should always stay for quick address access.

// CORE
const ohm = "0x64aa3364F17a4D01c6f1751Fd97C2BD3D7e7f1D5";
const gohm = "0x0ab87046fBb341D058F17CBC4c1133F25a20a52f";
const sohm = "0x04F2694C8fcee23e8Fd0dfEA1d4f5Bb8c352111F";
const weth = "0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2";

// STABLES
const dai = "0x6B175474E89094C44Da98b954EedeAC495271d0F";
const usdc = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48";
const usdt = "0xdAC17F958D2ee523a2206206994597C13D831ec7";
const mim = "0x99D8a9C45b2ecA8864373A26D1459e3Dff1e17F3";
const frax = "0x853d955aCEf822Db058eb8505911ED77F175b99e";
const lusd = "0x5f98805A4E8be255a32880FDeC7F6728C6568bA0";
const agEUR = "0x1a7e4e63778b4f12a199c062f3efdd288afcbce8";

// ANY TOKENS
const cvx = "0x4e3FBD56CD56c3e72c1403e103b45Db9da5B9D2B";
const crv = "0xD533a949740bb3306d119CC777fa900bA034cd52";
const angle = "0x31429d1856ad1377a8a0079410b297e1a9e214c2";
const lqty = "0x6DEA81C8171D0bA574754EF6F8b412F2Ed88c54D";
const fxs = "0x3432B6A60D23Ca0dFCa7761B7ab56459D9C964D0";
const btrfly = "0xC0d4Ceb216B3BA9C3701B291766fDCbA977ceC3A";
const xbtrfly = "0xCC94Faf235cC5D3Bf4bEd3a30db5984306c86aBC";
const alcx = "0xdbdb4d16eda451d0503b854cf79d55697f90c8df";
const toke = "0x2e9d63788249371f1dfc918a52f8d799f4a38c94";
const tribe = "0xc7283b66Eb1EB5FB86327f08e1B5816b0720212B";
const cvxcrv = "0x62B9c7356A2Dc64a1969e19C23e4f579F9810Aa7";
const wbtc = "0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599";
const steth = "0xae7ab96520DE3A18E5e111B5EaAb095312D7fE84";

// AAVE
const adai = "0x028171bCA77440897B824Ca71D1c56caC55b68A3";
const aweth = "0x030bA81f1c18d280636F32af80b9AAd02Cf0854e";
const ausdc = "0xBcca60bB61934080951369a648Fb03DF4F96263C";
const afrax = "0xd4937682df3C8aEF4FE912A96A74121C0829E664";

export const coins = {
    ohm,
    gohm,
    sohm,
    weth,
    dai,
    usdc,
    usdt,
    mim,
    frax,
    lusd,
    agEUR,
    cvx,
    crv,
    angle,
    lqty,
    fxs,
    btrfly,
    xbtrfly,
    alcx,
    toke,
    tribe,
    cvxcrv,
    steth,
    adai,
    aweth,
    ausdc,
    afrax,
};
