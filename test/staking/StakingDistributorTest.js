...
describe.skip("Distributor", () => {
    ...
    describe("constructor", () => {
        it("constructs correctly", async () => {
            ...
            let distributor = await DistributorContract.connect(owner).deploy(
                treasury.address,
                OHM.address,
                epochLength,
                nextEpochBlock
        ...
        it("does not accept 0x0 as OHM address", async () => {
            const nextEpochBlock = 1;
            await expect(
                DistributorContract.connect(owner).deploy(
                    treasury.address,
                    blackhole,
                    epochLength,
                    nextEpochBlock
    ...
    describe("distribute", () => {
        it("will do nothing if epoch is not complete", async () => {
            ...
            expect(BigNumber.from(await distributor.nextEpochBlock())).to.equal(nextEpochBlock);
        });
        ...
        it("only advances the epoch if there are no recipients", async () => {
            ...
            expect(await distributor.nextEpochBlock()).to.equal(
    ...
    describe("addRecipient", () => {
        ...
        it("can only be done by governor", async () => {
            ...
            await expect(distributor.connect(guardian).addRecipient(staking.address, 2975)).to.be
                .reverted;
        });
    });
