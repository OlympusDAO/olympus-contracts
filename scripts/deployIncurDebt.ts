...
async function main() {
    ...
    const curveStrategyFactory = await ethers.getContractFactory("CurveStrategy") as CurveStrategy__factory
    const curveStrategy = await curveStrategyFactory.deploy(curvePoolFactory, ohm, incurDebt.address)
    console.log("CURVE STRATEGY DEPLOYED AT", curveStrategy.address);

    const uniswapStrategyFactory = await ethers.getContractFactory("UniSwapStrategy") as UniSwapStrategy__factory
    const uniswapStrategy = await uniswapStrategyFactory.deploy(uniswapRouter, uniswapFactory, incurDebt.address, ohm)
    console.log("UNISWAP STRATEGY DEPLOYED AT", uniswapStrategy.address);

    const sushiStrategyFactory = await ethers.getContractFactory("UniSwapStrategy") as UniSwapStrategy__factory
    const sushiStrategy = await sushiStrategyFactory.deploy(sushiRouter, sushiFactory, incurDebt.address, ohm)
