 ...
 */
contract RebaseDirector is Ownable {
    ...
    function redirectTo(uint _principal, address _receiver) external returns ( bool ) {
        ...
        require(_recipient != 0x0, "Invalid recipient address");
        ...
        principal[msg.sender] = principal[msg.sender] + agnosticValue;

        // Add donor's principal to receivers debt as flat value
        debt[_receiver] = debt[_receiver] + _principal;
     ...
     */
    function claimableBalance(address _who) public returns (uint) {
        require(debt[who] == 0, "No claimable balance");

        return IsOHM(sOHM).balanceOf(address(this)) - debt[_who];
    }
