...
contract BondTeller is ITeller {
  ...
  function newBond(
    ...
    bonderInfo[_bonder].push(Bond({
      ...
      vested: _expires, 
      created: block.timestamp, redeemed: 0
    }));
  }
  ...
  function setFEReward(uint256 reward) external override {
  ...
  ) public override view returns (uint256 pending_) {
    for (uint256 i = 0; i < _indexes.length; i++) {
   ...
   *  @return pending_ uint256
   */
  function totalPendingFor(address _bonder) public override view returns (uint256 pending_) {
    Bond[] memory info = bonderInfo[_bonder];
    for (uint256 i = 0; i < info.length; i++) {
      pending_ = pending_.add(pendingFor(_bonder, i));
    }
   ...
   */
  function percentVestedFor(address _bonder, uint256 _index) public override view returns (uint256 percentVested_) {
