...
interface IGroDepositHandler {
    // deposit DAI to Gro
    function depositPwrd(uint256[3] memory inAmounts, uint256 minAmount, address _referral) external;
}

interface IGroWithdrawHandler {
    // withdraw DAI from Gro
    function withdrawAllSingle(bool pwrd, uint256 index, uint256 minAmount) external;
}
...
contract GroDAIAllocator is Ownable {
     ...
     */
    function deposit(uint amount, uint minAmount) public onlyPolicy() {
     ...
     *  @param minAmount uint
     */
    function withdraw(uint minAmount) public onlyPolicy() {
