...
contract TreasuryTokenMigrator {
     ...
     */
    function migrate() external onlyDAO {
        _migrateTokens();
    }
     ...
     */
    function addTokens(address[] memory _tokens, bool[] memory _reserveToken) external onlyDAO {
        require(_tokens.length == _reserveToken.length, "arrays length does not match");
