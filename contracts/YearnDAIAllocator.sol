...
interface IyDAIVault {
    // deposit dai to yearn
    function deposit(uint _amount) external returns(uint256);
    // withdraw dai from yearn
    function withdraw(uint _amount) external returns(uint256);
}

/**
 *  Contract deploys reserves from treasury into the Yearn
 */

contract YearnDAIAllocator is Ownable {
    ...
    IyDAIVault immutable yDAIVault; // Yearn DAI contract
    ...
    uint public limit;
    uint public newLimit;
    uint public limitChangeTimelockEnd;
    ...
    constructor ( 
        ...
        require( _yDAIVault != address(0) );
        yDAIVault = IyDAIVault( _yDAIVault );
    ...
    /**
     *  @notice withdraws asset from treasury, deposits asset into Yearn
     *  @param amount uint
     */
    function deposit(uint amount) public onlyPolicy() {
        ...
        // account for deposit
        uint value = treasury.valueOf( DAI, amount );
        accountingFor( amount, value, true );

        IERC20(DAI).approve(address(yDAIVault), amount); // approve yDAI pool to spend tokens
        
        yDAIVault.deposit( amount ); // deposit into Yearn
    }

    /**
     *  @notice withdraws yDAI from Yearn, then deposits asset into treasury
     *  @param amount uint
     */
    function withdraw(uint amount) public onlyPolicy() {
        yDAIVault.withdraw(amount); // withdraw from Yearn

        uint balance = IERC20( DAI ).balanceOf( address(this) ); // balance of asset withdrawn

        // account for withdrawal
        uint value = treasury.valueOf( DAI, balance );
     ...
     *  @param newMax uint
     */
    function lowerLimit( uint newMax ) external onlyPolicy() {
     ...
     *  @param newMax uint
     */
    function queueRaiseLimit( uint newMax ) external onlyPolicy() {
     ...
     */
    function raiseLimit() external onlyPolicy() {
    ...
    function exceedsLimit( uint amount ) public view returns ( bool ) {
        uint willBeDeployed = deployed.add( amount );
