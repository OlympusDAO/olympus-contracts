...
const debug = debug_1.debug('vm:block');
/* DAO account list */
...
function encodeReceipt(tx, receipt) {
    ...
    const type = ethereumjs_util_1.intToBuffer(tx.type);
...
async function generateTxReceipt(tx, txRes, blockGasUsed) {
    ...
    let txReceipt;
    let encodedReceipt;
    let receiptLog = `Generate tx receipt transactionType=${tx.type} gasUsed=${blockGasUsed.toString()} bitvector=${opcodes_1.short(abstractTxReceipt.bitvector)} (${abstractTxReceipt.bitvector.length} bytes) logs=${abstractTxReceipt.logs.length}`;
    if (!tx.supports(999)) {
        // Legacy transaction
        ...
        // EIP2930 Transaction
        txReceipt = Object.assign({ status: txRes.execResult.exceptionError ? 0 : 1 }, abstractTxReceipt);
        encodedReceipt = Buffer.concat([ethereumjs_util_1.intToBuffer(tx.type), rlp_1.encode(Object.values(txReceipt))]);
    }
    return {
        txReceipt,
        encodedReceipt,
